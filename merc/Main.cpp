...
// convenience functions for creating and destroying the validation message callback
// because the callback is an extension that is not loaded by default and must be manually fetched
...
private:
	...
	void pickPhysicalDevice() {
		...
		for (const auto& device : devices) {
			int score = deviceSuitabilityRating(device);
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		std::cout << ubo.light.x << ", " << ubo.light.z << std::endl;
		...
		//ubo.proj[1][1] *= -1; // flip y coord (legacy from opengl days)
